...
function doProm() {
    ...
    var cur = proms.splice(0, 1)[0];
    cur.then(function () {
        done++;
        var perc = (parseInt(done * 10000 / max) / 100) + '';
        if (perc != lastPerc) {
...
var csvStream = csv()
    .on("data", function (data) {
        if (i > 0) {
            ...
            row = [data[0], millis, data[7], data[8], data[9], data[16], data[23], data[49], data[50]];
            proms.push(db.addRatSighting(row));
        }
        i++;
        var perc = parseInt(i * 100 / max) + '';
        if (perc != lastPerc) {
