...
export function login(email, password) {
    ...
    return new Promise(function(resolve, reject) {
        post("/api/login", loginRequest)
            .then(function (response) {
                if (response == null) {
                    let loginResult = {success:false,err:"No server response",sessionID:null,permissionLevelOrdinal:0}
                    reject(loginResult)
                    return;
                }
                let result = JSON.parse(response.text)
                if (result.hasOwnProperty('err')) {
                    ...
                    let loginResult = {success:true,err:null,sessionID:result.sessionid,permissionLevelOrdinal:result.permLevel}
